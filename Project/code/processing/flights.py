...
class FlightProcessor(Processor):
    ...
    input = "flights"
    out = "flights"
    fields = ["date","airline","delay","carrier_delay","weather_delay","late_aircraft_delay","cancelled"]
    def process(self, in_, out):
        ...
        for row in reader:
            ...
            row["delay"] = mk_float(row['late_aircraft_delay']) + mk_float(row['carrier_delay'])
            out_r = {}
